...
std::ostream &operator<<(std::ostream &_Ostream, GraphMat<W, E>& _GraphMat)
{
    ...
    for (graph_size_t i = 0; i < length; ++i)
    {
        ...
        for (graph_size_t j = 0; j < length; ++j)
            _Ostream << ' ' << ((em[i][j] == INF) ? std::string("∞") : std::to_string(em[i][j]));
        _Ostream << std::endl;
    }
    return _Ostream;
}
...
template <typename W, typename E>
void GraphMat<W, E>::DFS(LinerLink<E>& _Lnk)
{
...
template <typename W, typename E>
void GraphMat<W, E>::DFS(graph_size_t _CurLocal,LinerLink<E>& _Lnk, bool * _Visit)
{
...
template <typename W, typename E>
void GraphMat<W, E>::BFS(LinerLink<E>& _Lnk)
{
    bool visit[_vc] = {false};
